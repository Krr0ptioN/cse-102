...
class RoadmapRepository(Repository):
    def create_roadmap(self, team_id: int, status: str, created_at: str) -> int:
    ...
    def set_roadmap_status(self, roadmap_id: int, status: str) -> None:
        with self.db.transaction() as conn:
            conn.execute(
    ...
    def add_roadmap_comment(
        self, roadmap_id: int, author: str, text: str, created_at: str, kind: str
    ) -> None:
        with self.db.transaction() as conn:
            conn.execute(
                """
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (roadmap_id, author, text, created_at, kind),
            )
    ...
    def create_task(
        ...
        with self.db.transaction() as conn:
            cur = conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (phase_id, title, weight, status, assignee_user_id, notes),
            )
            return int(cur.lastrowid)
